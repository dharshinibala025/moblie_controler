...
exports.sendTemporaryPasswordEmail = async ({ toEmail, name, tempPassword, role }) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 24px; color: #0F172A; max-width: 600px; margin: 0 auto; border: 1px solid #E2E8F0; border-radius: 12px;">
          ...
          <p>Your institutional account for the Smart Classroom Portal has been generated.</p>
          
          <div style="background-color: #F8FAFC; padding: 18px; border-radius: 8px; border: 1px solid #E2E8F0; margin: 20px 0;">
            <p style="margin: 0 0 8px 0;"><strong>Login Identifier:</strong> <code style="font-size: 14px; color: #0F172A;">${toEmail}</code></p>
            <p style="margin: 0;"><strong>Temporary Password:</strong> <code style="font-size: 18px; font-weight: bold; color: #2563EB;">${tempPassword}</code></p>
          </div>

          <h4 style="color: #334155; margin-bottom: 8px;">First Login Instructions:</h4>
          <ol style="padding-left: 20px; line-height: 1.6; color: #475569;">
            <li>Open the Smart Classroom App on your mobile device.</li>
            <li>Select your role tab (<strong>${role ? role.toUpperCase() : "STUDENT"}</strong>) and log in with your registered email and temporary password above.</li>
            <li>The app will automatically prompt you to create your new permanent password.</li>
            <li>Once updated, your login session will remain active on your registered device.</li>
          </ol>
    ...
    if (process.env.SMTP_APP_PASSWORD || process.env.SMTP_PASS) {
      ...
      logger.info(`[Development Mode] Temporary password for ${toEmail}: ${tempPassword}`);
    }
    return { success: true };
  } catch (err) {
